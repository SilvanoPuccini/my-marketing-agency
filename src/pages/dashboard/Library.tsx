...
function UploadModal({ files, onClose }: UploadModalProps) {
  ...
  // Fetch accounts when switching to new-piece mode
    ...
    >
      ...
      >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
                ...
                <select
                  value={newType}
                  onChange={(e) => setNewType(e.target.value as typeof PIECE_TYPES[number])}
                  style={inputStyle}
                >
                  {PIECE_TYPES.map((t) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 120, overflowY: 'auto' }}>
          {files.map((f) => (
            <div
              key={f.name}
              className="mono"
              style={{ fontSize: 11, color: 'var(--fg-3)', padding: '3px 0', borderBottom: '1px solid var(--line-1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
            >
              {f.name}
            </div>
          ))}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button
            onClick={onClose}
            style={{ padding: '7px 14px', fontSize: 12, fontWeight: 500, color: 'var(--fg-1)', borderRadius: 'var(--r-2)', border: '1px solid var(--line-2)', background: 'var(--bg-2)', cursor: 'pointer' }}
          >
            Cancelar
          </button>
          <button
...
export function Library() {
  ...
  const filtered = allFiles.filter((f) => {
    ...
    // Also honor the global store filter if set
    const matchesStore =
  ...
  function handleTypeFilter(label: string) {
    ...
    setPage(1)
    // also sync to global store for cross-page coordination
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: 'var(--fg-3)', margin: 0, fontSize: 13 }}>
            {filesQuery.isLoading
              ? 'Cargando archivos...'
              : `${allFiles.length} archivos · ordenados por ultima subida.`}
          </p>
        </div>
        ...
        {foldersQuery.isLoading ? (
          <div className="library-folders" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                style={{ background: 'var(--bg-1)', border: '1px solid var(--line-1)', borderRadius: 'var(--r-3)', padding: 14, height: 56 }}
              />
            ))}
          </div>
        ) : allFolders.length > 0 ? (
          <div className="library-folders" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
            {allFolders.map((f) => (
              <div
                key={f.account_id}
                style={{ background: 'var(--bg-1)', border: '1px solid var(--line-1)', borderRadius: 'var(--r-3)', padding: 14, cursor: 'pointer' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--line-3)' }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--line-1)' }}
              >
                <div style={{ fontWeight: 500, fontSize: 13 }}>{f.account_name}</div>
        ...
        {filesQuery.isLoading ? (
          <div className="library-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                style={{ background: 'var(--bg-1)', border: '1px solid var(--line-1)', borderRadius: 'var(--r-3)', overflow: 'hidden' }}
              >
          ...
          <div style={{ padding: '48px 0', textAlign: 'center', color: 'var(--fg-3)', fontSize: 14 }}>
            {allFiles.length === 0 ? 'No hay archivos en la biblioteca todavia.' : 'Ningun archivo coincide con los filtros.'}
          </div>
        ) : (
          <div className="library-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
            {visible.map((a) => (
              ...
              >
                ...
                >
                  ...
                  {isVideo(a) ? (
                    <div style={{ width: 36, height: 36, borderRadius: 999, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.16)', display: 'grid', placeItems: 'center', color: '#fff' }}>
                      &#9654;
                    </div>
                  ) : null}
                </div>
